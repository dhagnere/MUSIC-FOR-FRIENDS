  ...
  const onDelete = async (name) => {
    try {
      ...
      if (response.data.success) {
        toast.success("Playlist effacée avec succès");
        dispatch(
          SetSelectedPlaylist({
            name: "All Songs",
            songs: allSongs,
      ...
      dispatch(HideLoading());
      toast.error("Hum quelque chose s'est mal passé");
    }
  };
  ...
  return (
    <div>
      <div className="flex justify-between w-full">
        <h1 className="text-secondary text-2xl">Vos "playlist" personnelles</h1>
        <h1
